...
from TengenUzui import BOT_USERNAME, MESSAGE_DUMP, MONGO_DB_URI, app
from TengenUzui.utils.custom_filters import PREFIX_HANDLER
...
_DATABASE: AgnosticDatabase = _MGCLIENT["TengenUzuiAnime"]
...
DOWN_PATH = "TengenUzui/downloads/"
    ...
    async def wrapper(_, message: Message):
            ...
            try:
                ...
                if nut - out < 1.2:
                    ...
                    if USER_WC[user] == 3:
                        ...
                        await clog("TengenUzui", f"UserID: {user}", "SPAM")
                    if USER_WC[user] == 5:
                        ...
                        await clog("TengenUzui", f"UserID: {user}", "BAN")
                        return
        ...
        try:
            ...
            try:
                ...
                await clog("TengenUzui", e, "FAILURE", msg=message)
        ...
        if user in BOT_OWNER or user == int(cqowner):
            ...
            try:
                ...
                try:
                    ...
                    await clog("TengenUzui", e, "FAILURE", cq=c_q)
        else:
            if cqowner_is_ch:
                if user_valid:
                    try:
...
example: /anime Sword Art Online"""
    ...
    try:
        ...
        await clog("TengenUzui", title_img, "LINK", msg=message)
...
example: /manga Sword Art Online"""
    ...
    try:
        ...
        await clog("TengenUzui", pic, "LINK", msg=message)
        await client.send_photo(gid, failed_pic, caption=finals_, reply_markup=buttons)
...
async def character_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await clog("TengenUzui", img, "LINK", msg=message)
...
example: /airing Sword Art Online"""
    ...
    try:
        ...
        await clog("TengenUzui", coverImg, "LINK", msg=message)
...
This material is marked 18+ and not allowed in this group""",
    ...
    try:
        ...
        await clog("TengenUzui", pic, "LINK", msg=cq)
        await cq.edit_message_media(
...
➠ *TengenUzui provides you the best anime-based commands including anime news and much more!*
